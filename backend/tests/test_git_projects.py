...
from fbuild_backend.services.git_projects import list_project_branches, sync_project_workspace


class GitProjectsServiceTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_sync_clones_project_and_lists_remote_branches(self) -> None:
        project = create_project(str(self.remote_dir))

        synced_project = sync_project_workspace(project)
        branches = list_project_branches(synced_project)

        self.assertEqual(synced_project.default_branch, "main")
        self.assertEqual([branch.name for branch in branches], ["feature/login", "main"])
        self.assertTrue((Path(synced_project.workspace_path) / ".git").exists())
